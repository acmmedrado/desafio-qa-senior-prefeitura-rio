...
| Detalhe inexistente deve permitir recuperacao | Funcional/erro | `test_get_unknown_service_returns_404_instead_of_server_error`, `test_get_unknown_service_handles_adversarial_ids_without_server_error`, `test_service_detail_handles_encoded_injection_ids_without_server_error` | BUG-001 |
| Busca nao deve aceitar entrada invalida | Validacao | `test_search_rejects_empty_query`, `test_search_rejects_blank_query`, `test_search_rejects_missing_query_field`, `test_search_rejects_non_json_content_type` | BUG-002 |
...
| Endpoints protegidos devem exigir token | Seguranca | `test_favorite_requires_authorization`, `test_favorite_rejects_invalid_token`, `test_recommendations_requires_authorization`, `test_recommendations_rejects_invalid_authorization_variants` | BUG-005 |
| Webhook deve validar integridade do payload | Seguranca | `test_webhook_rejects_missing_signature`, `test_webhook_rejects_invalid_signature`, `test_webhook_rejects_signature_created_with_wrong_secret`, `test_webhook_rejects_signature_from_different_payload`, `test_webhook_rejects_malformed_signature_scheme` | BUG-004 |
| API nao deve vazar informacao sensivel em entradas hostis | Seguranca | `tests/test_security.py` | Quality gate + BUG-001, BUG-004 |
...
- Quality gate: 49 testes passando.
- Release gate: 10 falhas bloqueantes.
- Diagnostico nao bloqueante: 13 falhas conhecidas.
